...
export default function PullAvatar({ item, maxPity, rarity }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* Tooltip on hover */}
      <div className="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-card text-white text-xs px-2 py-1 rounded opacity-0 group-hover/avatar:opacity-100 transition-opacity duration-200 whitespace-nowrap z-20 border border-slate-600">
        {item.name}
      </div>
    </div>
  );
